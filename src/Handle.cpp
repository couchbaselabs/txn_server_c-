...
#undef _setcb
    ...
    Handle::Handle(const txnService::conn_info* conn_info) :
            conn_info(conn_info),
            instance(NULL)

    {
        cout <<"Creating the Handle. In between stuff"<< std::endl;
...
#define cstr_ornull(s) \
    ...
    {
        cout <<"Handle connect initiated"<< std::endl;
        // Gather parameters
        lcb_error_t the_error;
        instance = NULL;
        std::string connstr;
        if (!create_opts.v.v3.connstr) {
            cout <<"If clause for create_opts.v.v3.connstr"<< std::endl;
            if(conn_info->handle_ssl()) {
                cout <<"If clause for handle_ssl"<< std::endl;
                connstr += std::string("couchbases://") + conn_info->handle_hostname();
                connstr += std::string("/") + conn_info->handle_bucket();
                connstr += std::string("?certpath=");
                connstr += std::string(conn_info->handle_cert());
                certpath = "";
                connstr += std::string("&");
            } else {
                cout <<"Else clause for handle_ssl"<< std::endl;
                connstr += std::string("couchbase://") + conn_info->handle_hostname();
                connstr +=  std::string("/") + conn_info->handle_bucket();
                connstr += std::string("?");
            }
            cout <<"Done with I/Else clause for handle_ssl"<< std::endl;
            connstr += "detailed_errcodes=1&";
            create_opts.v.v3.connstr = cstr_ornull(connstr);
            create_opts.v.v3.passwd = cstr_ornull(conn_info->handle_password());
        }
/*
//Code below pulled from Daemon.cpp/
        cout <<"Code below pulled from Daemon.cpp"<< std::endl;
        lcb_error_t err;
        lcb_io_opt_t ret;
        std::string s_ioSymbol;
        std::string s_ioDLL;
        lcb_create_io_ops_st ioCreationOptions;
        cout <<"Initializing Code below pulled from Daemon.cpp"<< std::endl;
        ...
        cout <<"Creating IOPS structure"<< std::endl;
        ...
        cout <<"Creating lcb_create_io_ops"<< std::endl;
...
*/
        ...
        cout <<"Creating lcb_create:" << "the_error: " << the_error << std::endl;
        ...
        cout <<"Creating lcb_cntl LCB_CNTL_FETCH_MUTATION_TOKENS"<< std::endl;
        ...
        cout <<"Creating lcb_cntl LCB_CNTL_DETAILED_ERRCODES"<< std::endl;


        lcb_set_bootstrap_callback(instance, cb_config);
        cout <<"Done with lcb_set_bootstrap_callback"<< std::endl;

        lcb_set_cookie(instance, this);
        cout <<"Done with lcb_set_cookie"<< std::endl;

        wire_callbacks(instance);
        cout <<"Done with wire_callbacks"<< std::endl;
        ...
        cout <<"Done with lcb_connect"<< std::endl;

        lcb_wait3(instance, LCB_WAIT_NOCHECK);
        cout <<"Done with lcb_wait3"<< std::endl;
        ...
        cout <<"Done with lcb_get_bootstrap_status"<< std::endl;
